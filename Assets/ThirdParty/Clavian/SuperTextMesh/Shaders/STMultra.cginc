...
void dropShadow(inout fixed4 col, v2f i, inout float zed)
{
    if(col.a!=1.0)
    {
        ...
        float2 os = float2(sin((_DropshadowAngle/360.0)*doublepi), cos((_DropshadowAngle/360.0)*doublepi)) * _DropshadowDistance * texelFix / 128  * _MainTex_TexelSize.y * 256 * (min(i.scale.x,i.scale.y)/max(i.scale.x,i.scale.y)) * ratio(i.quality) /  max(i.scale.x,i.scale.y);
        ...
        */
        ...
        shadowCol.a *= offsetClip(i.dist + float4(os.xy, 0,0));
        //shadowCol.a*=saturate(offsetClip(i.dist + float4(os.xy, 0,0)));


        //}
        col = lerp(shadowCol,col,col.a);
        if(col.a<=0)
        discard;
        //since zwrite is on, we want to discard pixels so as to not
        //occlude anything with this quad's presence in depth buffer
