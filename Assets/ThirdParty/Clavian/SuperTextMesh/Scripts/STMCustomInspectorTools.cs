		...
		*/
		if(mat.GetTag("STMUberShader2", true, "Null") == "Yes")
		{
			...
			EditorGUI.indentLevel++;
				//for now, no SDF mode for the ultra shader
				...
				if(mat.GetInt("_Effect") == 1)
				{
					...
					if(mat.HasProperty("_DropshadowType"))
					{
						mat.SetInt("_DropshadowType",
							EditorGUILayout.IntPopup("Type", mat.GetInt("_DropshadowType"), new[] { "Angle", "Position" }, new[] { 0, 1 }));
						...
						{
							if(mat.HasProperty("_DropshadowAngle3"))
							{
								mat.SetVector("_DropshadowAngle3",
									EditorGUILayout.Vector3Field("Dropshadow Angle3", mat.GetVector("_DropshadowAngle3")));
							}
						}
					}
					EditorGUI.indentLevel--;
